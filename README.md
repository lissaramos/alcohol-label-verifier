...
- **Bulk import via CSV or JSON.** Instead of entering every application by hand, accept a manifest file that lists each label's fields and references its image file(s), auto-populating the queue in one shot. This pairs directly with the async job queue above — getting 200-300 entries in without manual re-typing only helps if the backend can also actually process that many without stalling on one item at a time.
